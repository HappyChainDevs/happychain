...
export interface EventBus<T extends EventSchema> {
    on<Key extends keyof T>(key: Key, handler: EventHandler): () => void
    off<Key extends keyof T>(key: Key, handler: EventHandler): void
    once<Key extends keyof T>(key: Key, handler: EventHandler): void
    emit<Key extends keyof T>(key: Key, payload: T[Key]): void
    clear(): void

    // compatibility methods
    removeAllListeners(): void
    addListener<Key extends keyof T>(key: Key, handler: EventHandler): () => void
    removeListener<Key extends keyof T>(key: Key, handler: EventHandler): void
}
...
export function eventBus<TDefinition extends EventSchema>(config: EventBusOptions): EventBus<TDefinition> {
    ...
    function registerPortListener(_port: MessagePort | BroadcastChannel) {
        ...
        config.logger?.log(`[EventBus] Port initialized ${config.mode}=>${config.scope}`, location.origin)
    }
    ...
    const emit: EventBus<TDefinition>['emit'] = (key, payload) => {
        if (!port) {
            config.logger?.warn(`[EventBus] Port not initialized ${config.mode}=>${config.scope}`, location.origin)
        }
