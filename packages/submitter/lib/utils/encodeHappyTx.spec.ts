import { describe, expect, test } from "bun:test"
import type { Hex } from "viem"
import type { HappyTx } from "#lib/tmp/interface/HappyTx"
import { decodeHappyTx } from "./decodeHappyTx"
import { encodeHappyTx } from "./encodeHappyTx"

describe("encode", () => {
    test("should correctly encode tx with empty dynamic data", () => {
        const tx: HappyTx = {
            account: "0x1234567890123456789012345678901234567890",
            dest: "0x2345678901234567890123456789012345678901",
            payer: "0x3456789012345678901234567890123456789012",
            value: 1000000000000000000n, // 1 ETH (10^19 in wei)
            nonceTrack: 1234n,
            nonceValue: 5678n,
            maxFeePerGas: 2000000000n, // (2 * 10^9) wei
            submitterFee: 100000000n, // (10^8) wei
            gasLimit: 1000000n,
            validateGasLimit: 800000n,
            executeGasLimit: 800001n,
            validatePaymentGasLimit: 800002n,
            callData: "0x",
            validatorData: "0x",
            extraData: "0x",
        }

        const expected: Hex =
            ""
        const encoded = encodeHappyTx(tx)
        expect(encoded).toBe(expected)
    })

    test("should correctly encode tx with all fields", () => {
        const tx: HappyTx = {
            account: "0x1234567890123456789012345678901234567890",
            dest: "0x2345678901234567890123456789012345678901",
            payer: "0x3456789012345678901234567890123456789012",
            value: 1000000000000000000n, // 1 ETH (10^19 in wei)
            nonceTrack: 1234n,
            nonceValue: 5678n,
            maxFeePerGas: 2000000000n, // (2 * 10^9) wei
            submitterFee: 100000000n, // (10^8) wei
            gasLimit: 1000000n,
            validateGasLimit: 800000n,
            executeGasLimit: 800001n,
            validatePaymentGasLimit: 800002n,
            callData: "0x1234",
            validatorData: "0x9abcd",
            extraData: "0xdef0",
        }

        const expected: Hex =
            ""
        const encoded = encodeHappyTx(tx)
        expect(encoded).toBe(expected)
    })

    test("should correctly encode tx with big dynamic fields", () => {
        const tx: HappyTx = {
            account: "0x1234567890123456789012345678901234567890",
            dest: "0x2345678901234567890123456789012345678901",
            payer: "0x3456789012345678901234567890123456789012",
            value: 1000000000000000000n, // 1 ETH (10^19 in wei)
            nonceTrack: 1234n,
            nonceValue: 5678n,
            maxFeePerGas: 2000000000n, // (2 * 10^9) wei
            submitterFee: 100000000n, // (10^8) wei
            gasLimit: 1000000n,
            validateGasLimit: 800000n,
            executeGasLimit: 800001n,
            validatePaymentGasLimit: 800002n,
            callData:
                "0x40c10f190000000000000000000000004bc8e81ad3be83276837f184138fc96770c1429700000000000000000000000000000000000000000000000000038d7ea4c68000",
            validatorData:
                "0x827a29d9e7e5e37adc8ae5ead7993f7d354da82a35a05da3fef21d133e22082f376916126bfece3e226c3a9bfb55354783deb43b58989d0a29ec53b4f36560cc1b",
            extraData: "0xdef0def0def0def0def0def0def0def0def0def0def0def0def0def0def0def0def0",
        }

        const expected: Hex =
            ""
        const encoded = encodeHappyTx(tx)
        expect(encoded).toBe(expected)
    })

    test("should both encode and decode as expected", () => {
        const happyTx: HappyTx = {
            account: "0x1234567890123456789012345678901234567890",
            dest: "0x2345678901234567890123456789012345678901",
            payer: "0x3456789012345678901234567890123456789012",
            value: 1000000000000000000n, // 1 ETH (10^19 in wei)
            nonceTrack: 1234n,
            nonceValue: 5678n,
            maxFeePerGas: 2000000000n, // (2 * 10^9) wei
            submitterFee: 100000000n, // (10^8) wei
            gasLimit: 1000000n,
            validateGasLimit: 800000n,
            executeGasLimit: 800001n,
            validatePaymentGasLimit: 800002n,
            callData:
                "0x40c10f190000000000000000000000004bc8e81ad3be83276837f184138fc96770c1429700000000000000000000000000000000000000000000000000038d7ea4c68000",
            validatorData:
                "0x827a29d9e7e5e37adc8ae5ead7993f7d354da82a35a05da3fef21d133e22082f376916126bfece3e226c3a9bfb55354783deb43b58989d0a29ec53b4f36560cc1b",
            extraData: "0xdef0def0def0def0def0def0def0def0def0def0def0def0def0def0def0def0def0",
        }
        const encoded = encodeHappyTx(happyTx)
        const decoded = decodeHappyTx(encoded)
        expect(decoded).toEqual(happyTx)
    })
})
