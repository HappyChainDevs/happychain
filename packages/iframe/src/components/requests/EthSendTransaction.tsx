...
import { type Hex, type RpcTransactionRequest, formatEther } from "viem"
import { useEstimateGas } from "wagmi"
import { getCurrentChain } from "#src/state/chains.js"
import { queryClient } from "#src/tanstack-query/config.js"

import { requestLabels } from "#src/constants/requestLabels.js"
import { StorageKey, storage } from "#src/services/storage.js"
...
export const EthSendTransaction = ({
    ...
    const shouldEstimateGas = !tx.maxFeePerGas
     ...
     */
    const normalizeAndAcceptTransaction = useCallback(
        (tx: RpcTransactionRequest) => {
            ...
            if (shouldEstimateGas) {
                if (tx.gasPrice) {
                    ...
                    updatedTx.maxFeePerGas = (gasEstimate?.toString() as Hex) ?? "0x0"
        ...
        [accept, queryKey, shouldEstimateGas, gasEstimate, method],
    )

    // biome-ignore lint/correctness/useExhaustiveDependencies: only need to observe the tx field changes
    const txTypeLabel = useMemo(() => classifyTxType(tx), [tx])
    const formattedTxValue = useMemo(() => formatEther(BigInt(tx.value ?? "0")), [tx.value])
                ...
                BigInt(
                    shouldEstimateGas
                        ? isGasEstimateLoading
                            ? "0x0"
                            : (gasEstimate?.toString() ?? "0x0")
                        : (tx.maxFeePerGas ?? "0x0"),
                ),
            ),
        [shouldEstimateGas, gasEstimate, tx.maxFeePerGas, isGasEstimateLoading],
    )
    const formattedMaxPriorityFee = useMemo(
        () => formatEther(BigInt(tx.maxPriorityFeePerGas ?? "0x0")),
                ...
                <div className="flex grow flex-col gap-4 overflow-y-auto rounded-lg bg-base-200 p-4">
                    <div className="flex flex-col gap-6">
                        <div className="flex flex-col items-center gap-2">
                            <span className="text-sm text-neutral-content">SENDING</span>
                            <span className="text-3xl font-bold">{formattedTxValue} HAPPY</span>
                        </div>
                        ...
                        <details className="collapse collapse-arrow bg-base-100 items-center">
                            ...
                            <div className="collapse-content flex flex-col gap-3">
                                <div className="flex flex-col gap-2">
                                    {/* tx type */}
                                    <div className="flex justify-between">
                                        <span className="text-sm text-neutral-content font-mono">Type</span>
                                        <span className="font-mono text-sm">{txTypeLabel}</span>
                                    </div>
                                    {/* Gas Details */}
                                    <span className="text-sm text-neutral-content font-mono font-bold">
                                        Gas Details
                                    </span>
                                    <div className="flex items-center justify-between">
                                        <span className="text-sm text-neutral-content font-mono">MaxFeePerGas</span>
                                        <span className="font-mono text-sm">{formattedMaxFee} HAPPY</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-sm text-neutral-content font-mono">PriorityFee</span>
                                        <span className="font-mono text-sm">{formattedMaxPriorityFee} HAPPY</span>
                                    </div>
                                </div>
