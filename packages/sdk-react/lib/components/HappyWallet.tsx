...
export function HappyWallet() {
	...
	useEffect(() => {
		const off = messageBus.on("modal-toggle", (state) => {
			setIsOpen(state);
		});
		return () => off();
	}, []);

	const iframeWrapperClasses = useMemo(
		() => getClasses(user, isOpen),
		[user, isOpen],
	);
	...
	return (
		<>
			<div
				className={clsx("hc-fixed hc-top-0 hc-right-0", iframeWrapperClasses)}
			>
				<Iframe />
			</div>
		</>
