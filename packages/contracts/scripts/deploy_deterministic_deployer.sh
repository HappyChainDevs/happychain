...
# After PR review, maybe fetch this from .env file, assuming the RPC is up and running
JSON_RPC="http://localhost:8545"

# Uncomment the following lines to spin up a docker container with geth-clique
...
# Uncomment below code when deploying fresh contract on a new chain, to verify that it works
...
# Uncomment the following lines to shutdown the docker container (if started)
## Shutdown Parity
#docker container stop deployment-proxy-geth
