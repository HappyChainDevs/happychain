...
interface SkeuoButtonElementProps
    extends SkeuoButtonBaseProps,
        Omit<HTMLArkProps<"button">, keyof SkeuoButtonBaseProps> {
    href?: undefined
}

interface SkeuoAnchorElementProps extends SkeuoButtonBaseProps, Omit<HTMLArkProps<"a">, keyof SkeuoButtonBaseProps> {
    href: string
}
...
const SkeuoButtonShell = ({ className, intent, scale, depth, children }: SkeuoButtonShellProps) => {
    return (
        <div className={recipeSkeuoButton.parent({ className })}>
...
export const SkeuoButton = forwardRef((props: SkeuoButtonProps, ref: React.ForwardedRef<HTMLElement>) => {
    const id = useId()
    const { intent, scale, depth, alignItems, justifyContent, className, children, ...rest } = props

    if (rest?.href) {
        return (
            <SkeuoButtonShell className={className}>
                <span
                    className={recipeSkeuoButton.label({ intent, scale, justifyContent, alignItems })}
                    id={id}
                    aria-hidden={true}
                    data-part="label"
                >
