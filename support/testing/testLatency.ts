...
import { type ExecuteSuccess, type ExecuteOutput, BoopClient, computeBoopHash, Onchain } from "@happy.tech/boop-sdk";
...
async function run() {
    ...
    for (let i = 0; i < numTransactions; i++) {
        ...
        // `tx` is defined here, but might be undefined in catch if `createAndSignMintTx` fails
        // before `tx` is fully assigned, though unlikely with `await`.
        ...
        const transactionExecutionPromise = new Promise<void>(resolve => {
            setTimeout(async () => {
                ...
                let executeResult: ExecuteOutput;
                let status: string;
                let txHash: string;
                ...
                try {
                    ...
                    if (executeResult.status === Onchain.Success) {
                        ...
                        console.log(`Boop: https://explorer.testnet.happy.tech/tx/${txHash}`);
                    } else {
                        // Use the already computed boopHash for error logging
                        console.error(`Execute not successful for boop ${computedBoopHash}: ${stringify(executeResult)}`);
                    }
                } catch (error) {
                    console.error(`Error executing boop : ${stringify(error)}`);
                } finally {
                    const end = performance.now();
                    const latency = (end - start).toFixed(2);
                    // Append data to CSV file immediately after each transaction completes
                    appendFileSync(CSV_FILE, `${latency},${status!},${txHash!}\n`);
