...
guildsApi.get("/:id", zValidator("param", GuildIdParamSchema), async (c) => {
    try {
...
guildsApi.patch(
    ...
    zValidator("json", GuildUpdateRequestSchema),
    async (c) => {
        try {
            const { id } = c.req.valid("param")
...
guildsApi.get("/:id/members", zValidator("param", GuildIdParamSchema), async (c) => {
    try {
        const { id } = c.req.valid("param")
...
guildsApi.delete("/:id/members/:userId", zValidator("param", GuildMemberParamSchema), async (c) => {
    try {
        const { id, userId } = c.req.valid("param")
...
guildsApi.get("/users/:id/guilds", async (c) => {
    try {
