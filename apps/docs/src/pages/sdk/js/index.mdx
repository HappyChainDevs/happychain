...
What happens under the hood? When a session key is approved, the wallet creates a key and saves it in the wallet's
local storage. The session key's address is registered with the user's wallet onchain. The key itself never leaves
the wallet, so it cannot leak.

Session keys can be revoked in the wallet settings, under "Permissions".
...
If you would like to customize the look, the default styles can be completely disabled by passing 
the `disable-styles`  property to the component, and setting `overrideBadgeStyles` to true 
in [`loadHappyWallet`].
