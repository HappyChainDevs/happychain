...
Here's a diagram of how a Boop flows between the locations we mentionned in the previous section.
...
an [account registy]. This is an important technique, as it is the only way to ascertain the implementation used by an
...
account from the registry when they independently upgrade. Also note our current implementation doesn't do this (but is
