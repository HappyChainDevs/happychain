...
and should be considered as the canonical specification for behaviour. This page extracts the essential information in a
...
Please checkout the [Contracts Overview][overview] of the Architecture page for background information. We reprodue the
...
This structure represent a boop (a smart transaciton in the Boop stack). The following excerpt is copied from the
[Types.sol] file.
...
file][Encoding.sol], which you can vendor if you need to encode/decode boops on the chain yourself. Our JavaScript/TypeScript
[Client SDK](/boop/sdk) expose [`decodeBoop`] and [`encodeBoop`] functions that do the same.
...
[`Utils.sol`][Utils.sol] to read from the dictionary on the contract size. Our JavaScript/TypeScript [Client
SDK](/boop/sdk) exposes an [`encodeExtraData`] function to create extra data from a client.
...
The [Boop Client SDK](/boop/sdk) exposes an [`computeBoopHash`] function to compute a Boop's hash. You can see how this
computation is done on the contract side in the `validate` function of our [`HappyAccount`][HappyAccount] account
implementation.
...
1. Validate the gas price, check the paymaster's staking balance (if a paymaster is specified), validate and update the
account nonce.

2. Call the account to validate the boop. (`IAccount.validate`, see below)

3. Call the paymaster to validate payment if a paymaster is specified. (`IPaymaster.validatePayment`, see below)

4. Call the account to execute the call specified by the boop. (`IAccount.execute`, see below)

5. Collect payment from the paymaster or account, if the transaction is not sponsored by the submitter.
Payment is taken from the paymaster's stake or solicited from the account by calling `IAccount.payout`.
...
The user can use any account implementation of his choice that implement the [`IAccount`][IAccount] interface. We
provide our own [`HappyAccount`][HappyAccount] compliant implementation — note that despite the name, the implementation
...
Here are the core functions the account must implement, refer to [interface][iaccount-source] for more details, including
...
- `execute(Boop)` — This executes the operation specified by the boop. The function returns a structure indicating the
  outcome: whether the operation succeeded, reverted, or whether the account rejected the operation (typically because
  of malformed data). Since the function is not allowed revert, if it calls external contracts that may revert, it must
  catch the revert.
...
ERC-4337 account abstract. Those have you do incur an network roundtrip to collect a signature from a paymaster EOA
...
achieved, since a smart contract can call into the EntryPoint and every boop is given its own gas limit. This is
currently not a priority feature, but reach out at hello@happy.tech if interested.
