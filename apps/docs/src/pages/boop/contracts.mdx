...
Just like an EVM transaction ban be identified by its hash, so can a Boop. The boop hash is straightforwardly computed as
...
1. Validates the gas price, checks the paymaster's staking balance (if a paymaster is specified), validates and updates the
account nonce.

2. Calls the account to validate the boop. (`IAccount.validate`, see below)

3. Calls the paymaster to validate payment if a paymaster is specified. (`IPaymaster.validatePayment`, see below)

4. Calls the account to execute the call specified by the boop. (`IAccount.execute`, see below)

5. Collects payment from the paymaster or account, if the transaction is not sponsored by the submitter.
