...
Please checkout the [Submitter Overview][overview] of the Architecture page for background information.
...
When the sumitter receives a boop for onchain submission (`submit` or `execute`), the boop goes through the following
stages:
...
3. We submit the boop to the chain, then move to wait for the receipt. We say that the boop is "pending" — this is an
   ...
   Depending on its configuration, the submitter may attempt to resubmit the boop if it's not included fast enough. If will
...
The submitte comes with a plethora of configuration options which are controlled via environment variable (which you can
...
Only options (1) and (5), the most restrictive, offer a complete solution. The combination of options (3) and (4)
probably provide adequate protection on a low-fee blockchain, with option (2) adding even more protection, at the cost
of complexity and requiring access to a node's tracing API.
