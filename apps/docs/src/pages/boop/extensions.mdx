...
- Validation extension are triggered at the account's discretion in his [`validate(Boop)`][account-overview] function.
  They enable permissioning certain operations differently. Validation extension implement their own version of
  `validate(Boop)`, with the same signature and specification.

[account-overview]: /boop/contracts#account-contract

- Execution extensions are triggered at the account's discretion in his [`execute(Boop)`][account-overview] function.
  They enable customizing how certain operations are run. Execution extension implement their own version of
  `execution(Boop)`, with the same signature and specification.

A typical account implementationg like the [`HappyAccount`] will have a standard validation and execution path,
usually verifying some kind of signature for validation and carrying off the call specified in the boop's `dest`,
`callData` and `value` fields for execution. Extensions allows it to do different things in certain cases.
...
- The [`SessionKeyValidator`] enables account to authorize transactions to specific addresses to be signed by another EOA.
  We use this to implement [session keys] in the [Happy Wallet], enable a user to authorize an app to make transactions on
  his behalf, but the principle can be using for any kind of simple delegation with target address as the single criteria.
...
make a call to the `validate` or `executed` function of the extension, respectively.
...
3. You can now send boops with `dest` as the authorized target address. Specify the session key validator address as the
   value for `ICustomValidator.VALIDATOR_KEY` (`0x000001`). The account will use the session key contract to validator the
   call.
...
2. You can now use the batch call executor by specifying the batch call executor address as the value for
   ...
   `callData` fields of the boopt itself are ignored and should be zeroed/empty.
   ...
   length of an `extraData` key's value is encoded on 3 bytes anyway (allowing for up to 16MB values, which is more than
