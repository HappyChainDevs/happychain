...
export const blockExplorerKeys = {
    ...
    addresses: {
        /**
         * Base key for all address related queries
         */
 ...
 */
export const createBlockExplorerUrl = (path: string): string => {
 ...
 */
export const fetchFromBlockExplorer = async <T>(path: string): Promise<T> => {
    const url = createBlockExplorerUrl(path)
...
/**
 * Hook for fetching smart contract data
 * @param address - The contract address to fetch data for
...
export function useSmartContract(address: Address, options = {}) {
    return useQuery({
        ...
        queryFn: () => fetchFromBlockExplorer<{ name: string }>(`smart-contracts/${address}`),
 ...
 * @param hash - The transaction hash to fetch data for
 * @param options - Optional React Query configuration options
 * @returns React Query result object
 */
...
/**
 * Hook for fetching address details
