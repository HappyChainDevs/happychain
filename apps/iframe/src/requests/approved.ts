...
export async function dispatchHandlers(request: PopupMsgs[Msgs.PopupApprove]) {
    ...
    switch (request.payload.method) {
        ...
        case "eth_sendTransaction": {
            try {
                const preparedUserOpFromPopup = request.payload
                    .extraData satisfies ApprovedRequestPayload<"eth_sendTransaction">["extraData"]
                ...
                return await sendUserOp({
                    ...
                    preparedOp: preparedUserOpFromPopup,
                })
            } catch (error) {
                console.error(error)
                throw error
