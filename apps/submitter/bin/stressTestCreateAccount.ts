/**
 * Stress test for the createAccount endpoint.
 * Sends multiple concurrent requests to create accounts and measures performance.
 */

import { BoopClient, CreateAccount } from "@happy.tech/boop-sdk"
import { type Hex, stringify } from "@happy.tech/common"
...
`)
        ...
        for (let i = 0; i < remainingRequests; i++) {
            batchPromises.push(
                (async () => {
                    ...
                    const salt = `0x${(batchIndex * config.batchSize + i).toString(16).padStart(64, "0")}` as Hex
                    ...
                    try {
                        ...
                        const success =
                            result.status === CreateAccount.Success || result.status === "createAccountAlreadyCreated"

                        // Track the created account if successful
                        if (success && result.address) {
                            trackCreatedAccount(result.address)
                        }
    ...
    console.log(results.forEach((r) => console.log(r.success ? r.address : r.error)))
