/**
 * Stress test for the execute endpoint.
 * Sends multiple concurrent requests to execute boops and measures performance.
 */
...
const DEFAULT_CONFIG = {
    concurrentRequests: 500, // Lower default for execute since it's more resource-intensive
    batchSize: 50,
...
`)
    ...
    // First create an account to use for all execute requests
        ...
        for (let i = 0; i < remainingRequests; i++) {
            batchPromises.push(
                (async () => {
                    ...
                    // This helps avoid nonce conflicts when executing many transactions
                    const nonceValue = BigInt(i % 50)
                    const nonceTrack = BigInt(Math.floor(i / 50) + batchIndex * 10)

                    try {
                        // Create and sign a boop for execution
                        const boop = await createAndSignMintBoop(eoa, { account, nonceTrack, nonceValue })

                        // Calculate boopHash for logging
                        const chainId = 216n // Using testnet chain ID, adjust if needed
                        const boopHash = computeBoopHash(chainId, boop)
                        ...
                        results.push({
                            ...
                            boopHash: result.receipt?.boopHash || boopHash,
                            evmTxHash: result.receipt?.evmTxHash,
                        })
                    } catch (error) {
                        results.push({
                            success: false,
    ...
    // Show successful transaction hashes
    if (successCount > 0) {
        console.log("\n=== SAMPLE SUCCESSFUL TRANSACTIONS ===")
        results
            .filter((r) => r.success && r.evmTxHash)
            //   .slice(0, 5)
            .forEach((result, i) => {
                console.log(`Transaction ${i + 1}: ${result.evmTxHash}`)
            })
    }
