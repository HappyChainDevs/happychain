...
export async function waitForSubmitReceipt<account extends Account | undefined = undefined>(
    ...
    { txHash, happyTxHash, happyTx }: { txHash: Hex; happyTx: HappyTx; happyTxHash: Hex },
) {
    const receipt = await waitForTransactionReceipt(client, { hash: txHash, pollingInterval: 500 })
