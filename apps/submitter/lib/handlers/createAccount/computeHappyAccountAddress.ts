...
// This little dance makes sure we don't run into "using before initialization", which would occur otherwise.
 ...
 * This is used for staging deployments where the proxy code doesn't include metadata.
 */
...
async function fetchProxyCreationCode(): Promise<Hex> {
    try {
        ...
        return FALLBACK_PROXY_CREATION_CODE_NO_METADATA
    }
}
